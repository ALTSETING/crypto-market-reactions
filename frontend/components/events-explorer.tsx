  ...
  return (
    <section aria-labelledby="events-heading" className="mt-10">
      <div className="rounded-3xl border border-white/10 bg-slate-900/55 p-4 shadow-2xl shadow-black/20 backdrop-blur sm:p-6">
        <form key={currentQuery} onSubmit={submitSearch} role="search">
          ...
          <div className="mt-2 flex flex-col gap-2 sm:flex-row">
            ...
            <button className="h-[54px] shrink-0 rounded-2xl bg-emerald-400 px-6 text-sm font-bold text-slate-950 outline-none transition hover:bg-emerald-300 focus-visible:ring-2 focus-visible:ring-emerald-200" type="submit">Search</button>
          </div>
        </form>
        <p className="mt-2 text-xs text-slate-500" id="search-hint">Press Enter or select Search to update the results.</p>
        ...
        <form className="mt-5 grid gap-3 border-t border-white/8 pt-5 sm:grid-cols-2 lg:grid-cols-[1fr_160px_160px_auto]" key={`${searchParams.get("source")}-${searchParams.get("from")}-${searchParams.get("to")}`} onSubmit={applyAdvancedFilters}>
          ...
          <div className="flex items-end gap-2">
            <button className="h-[42px] rounded-xl bg-slate-100 px-4 text-sm font-bold text-slate-950 outline-none transition hover:bg-white focus-visible:ring-2 focus-visible:ring-emerald-300" type="submit">Apply</button>
      ...
      {data && data.totalPages > 0 && (
        <nav aria-label="Event result pages" className="mt-8 flex items-center justify-between rounded-2xl border border-white/10 bg-slate-900/50 p-3 sm:p-4">
          <button className="rounded-xl border border-white/10 px-4 py-2 text-sm font-semibold text-slate-200 outline-none transition enabled:hover:border-white/25 enabled:hover:bg-white/5 focus-visible:ring-2 focus-visible:ring-emerald-300 disabled:cursor-not-allowed disabled:opacity-35" disabled={data.page <= 1 || loading} onClick={() => replaceParams({ page: String(data.page - 1) })} type="button"><span aria-hidden="true">←</span> Previous</button>
          ...
          <button className="rounded-xl border border-white/10 px-4 py-2 text-sm font-semibold text-slate-200 outline-none transition enabled:hover:border-white/25 enabled:hover:bg-white/5 focus-visible:ring-2 focus-visible:ring-emerald-300 disabled:cursor-not-allowed disabled:opacity-35" disabled={data.page >= data.totalPages || loading} onClick={() => replaceParams({ page: String(data.page + 1) })} type="button">Next <span aria-hidden="true">→</span></button>
        </nav>
      )}
    </section>
  );
