...
describe("AI Search prototype", () => {
  it("renders examples, states baseline, Reaction V2 provenance, and disclaimer context", () => {
    ...
    expect(html).toContain("What is crypto staking?");
    expect(html).toContain("What are ETF inflows, and how does BTC react to them historically?");
  ...
  it("renders hybrid output as two clearly separated sections", async () => {
    ...
    expect(html).toContain("Combined explanation + historical database analysis");
    expect(html).toContain('aria-label="General explanation"');
    expect(html).toContain('aria-label="Historical database analysis"');
