  ...
  return (
    <section aria-label="AI Research search" className="mt-6 min-w-0 max-w-full sm:mt-9">
      ...
      <form className="mt-3 rounded-2xl border border-white/10 bg-slate-900/45 p-2 focus-within:border-emerald-400/40 focus-within:ring-4 focus-within:ring-emerald-400/[0.05]" onSubmit={submit}>
        ...
        <textarea
          className="block min-h-20 w-full min-w-0 resize-y bg-transparent px-3 py-2.5 text-base leading-6 text-white outline-none placeholder:text-slate-600 sm:min-h-16"
        ...
        <div className="flex items-center justify-between gap-3 border-t border-white/8 px-2 pt-2">
          ...
          <button className="min-h-10 shrink-0 rounded-xl bg-emerald-400 px-4 text-sm font-semibold text-slate-950 outline-none transition hover:bg-emerald-300 active:translate-y-px focus-visible:ring-2 focus-visible:ring-emerald-200 disabled:cursor-not-allowed disabled:opacity-45" disabled={state.kind === "loading" || question.trim().length < 3} type="submit">
      ...
      <div aria-live="polite" className={state.kind === "idle" ? "sr-only" : "mt-7 min-w-0 max-w-full"}>
        {state.kind !== "idle" && submittedQuestion && (
          <div className="mb-7 min-w-0">
            ...
            <p className="mt-1 break-words text-sm font-medium leading-6 text-slate-300 [overflow-wrap:anywhere]">{submittedQuestion}</p>
          </div>
        )}
...
export function ExampleQuestions({ expanded, onSelect, onToggle }: { expanded: boolean; onSelect: (question: string) => void; onToggle: () => void }) {
  return (
    <div className="mt-3">
      <button
        aria-controls="ai-example-questions"
        aria-expanded={expanded}
        className="inline-flex min-h-10 items-center gap-2 rounded-lg px-1 text-sm font-medium text-slate-500 outline-none transition hover:text-white focus-visible:ring-2 focus-visible:ring-emerald-300 motion-reduce:transition-none"
        onClick={onToggle}
        type="button"
      >
        Example questions
      ...
      <div aria-hidden={!expanded} className={`grid transition-[grid-template-rows,opacity] duration-150 motion-reduce:transition-none ${expanded ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`} id="ai-example-questions">
        <div className="min-h-0 overflow-hidden">
          <div className="grid gap-x-6 pb-1 pt-1 sm:grid-cols-2">
...
export function AiLoadingState() {
  return (
    <div aria-label="AI Research is analyzing" className="py-1" role="status">
...
export function AiMessage({ kind, label, message }: { kind: "warning" | "error"; label: string; message: string }) {
  return (
    <div className={`py-1 text-sm leading-6 ${kind === "error" ? "text-rose-200" : "text-amber-100"}`} role={kind === "error" ? "alert" : "status"}>
...
export function AiResult({ data }: { data: AiResearchSuccess }) {
  ...
  if (data.mode === "general") {
    return (
      <article aria-label="AI explanation" className="mx-auto min-w-0 max-w-[760px]">
        <ResultEyebrow>AI explanation</ResultEyebrow>
        <ProseAnswer text={data.answer} />
        <ResultDisclaimer>{compactDisclaimer(data.language)}</ResultDisclaimer>
      </article>
    );
  }
  ...
  return (
    <div className="min-w-0 max-w-full">
      <article aria-label="AI explanation" className="mx-auto min-w-0 max-w-[760px]">
        ...
        <ProseAnswer text={data.answer} />
        {data.historicalUnavailable && <p className="mt-6 text-sm text-slate-500">{data.historicalMessage ?? "Historical evidence is temporarily unavailable."}</p>}
        {!historicalData && <ResultDisclaimer>{compactDisclaimer(data.language)}</ResultDisclaimer>}
      </article>
      {historicalData && <div className="mt-9 border-t border-white/10 pt-8"><DatabaseResult data={historicalData} /></div>}
    </div>
  );
}
...
function ProseAnswer({ text }: { text: string }) {
  ...
  return (
    <div className="mt-3 min-w-0 space-y-4 break-words text-base leading-[1.65] text-white sm:text-[17px] sm:leading-[1.7] [overflow-wrap:anywhere]">
...
function ResultDisclaimer({ children }: { children: React.ReactNode }) {
  return <p className="mt-6 text-xs leading-5 text-slate-500">{children}</p>;
}

function compactDisclaimer(language: "en" | "uk"): string {
  return language === "uk" ? "Не фінансова порада." : "Not financial advice.";
}
...
function DatabaseResult({ data }: { data: AiSearchSuccess | AiHybridSuccess }) {
  const context = [
    ...
    data.intent.horizon ?? "All horizons",
  ...
  const requestedRow = data.result.kind === "multi_horizon"
    ...
    : null;
  const observationSummary = data.result.topicFilter
    ? `${data.result.topicFilter.independentEventCount} independent events · ${data.result.topicFilter.matchedSampleSize} matched articles`
    : `${sampleSize} observations`;

  return (
    <section aria-label="Historical evidence" className="min-w-0 max-w-full">
      {data.mode === "hybrid" && (
        <article aria-label="General explanation" className="mx-auto max-w-[760px] pb-8">
      ...
      <div className={data.mode === "hybrid" ? "border-t border-white/10 pt-8" : undefined}>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-full bg-emerald-400/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-emerald-300">Reaction V2</span>
        </div>
        ...
        {requestedRow && (
          <dl className="mt-5 flex flex-wrap gap-x-7 gap-y-3 rounded-xl bg-white/[0.035] px-4 py-3.5 text-sm">
            <InlineMetric label={`Median ${requestedRow.horizon}`} value={formatPercent(requestedRow.median)} tone={reactionTone(requestedRow.median)} />
            <InlineMetric label="Positive" value={formatPercent(requestedRow.positivePercent, false, 2)} />
            <InlineMetric label="Sample" value={String(requestedRow.sampleSize)} />
          </dl>
        )}
...
function InlineMetric({ label, tone = "text-white", value }: { label: string; tone?: string; value: string }) {
  return <div className="min-w-0"><dt className="text-[10px] font-semibold uppercase tracking-[0.1em] text-slate-600">{label}</dt><dd className={`mt-0.5 font-mono font-medium tabular-nums ${tone}`}>{value}</dd></div>;
}
...
export function CitationList({ citations, initialExpanded = false }: { citations: AiCitation[]; initialExpanded?: boolean }) {
  ...
  return (
    <section aria-labelledby="sources-heading" className="mt-8">
      <h4 className="text-sm font-semibold text-white" id="sources-heading">Sources</h4>
      <ol className="mt-3 space-y-1">
        {visibleCitations.map((citation, index) => (
          <li className="grid min-w-0 grid-cols-[1.5rem_1fr] gap-2 py-1.5 text-sm" key={citation.eventId}>
            <span aria-hidden="true" className="font-mono text-xs leading-6 text-slate-600">{index + 1}</span>
            <div className="min-w-0">
              <a className="inline-flex min-h-10 break-words py-1 leading-5 text-sky-200 outline-none hover:text-white focus-visible:rounded focus-visible:ring-2 focus-visible:ring-emerald-300 [overflow-wrap:anywhere]" href={citation.href}>{citation.title}</a>
              {citation.groupSize && citation.groupSize > 1 ? <p className="text-[11px] text-slate-500">{citation.groupSize} related articles</p> : null}
            </div>
          </li>
        ))}
      </ol>
      {citations.length > 5 && (
        <button aria-expanded={expanded} className="mt-2 min-h-10 rounded-lg px-1 text-sm font-medium text-slate-500 outline-none transition hover:text-white focus-visible:ring-2 focus-visible:ring-emerald-300" onClick={() => setExpanded((open) => !open)} type="button">
      ...
      <table className="w-full min-w-[460px] border-collapse text-sm">
        ...
        <thead>
          <tr className="border-b border-white/10 text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">
            ...
            <th className="py-2 pl-3 text-right" scope="col">Positive</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row) => (
            <tr className="border-b border-white/8 last:border-b-0" key={row.horizon}>
              ...
              <td className="py-3 pl-3 text-right font-mono tabular-nums text-white">{formatPercent(row.positivePercent, false, 2)}</td>
            </tr>
          ))}
        </tbody>
      </table>
