...
class TransientRouterError extends Error {}
...
const LIVE_PATTERN = /\b(?:current|right\s+now|live|latest|today(?:'s)?|now)\b[^?]{0,50}\b(?:price|news|flows?|inflows?|outflows?|market|regulatory\s+status)|\b(?:price|news|flows?|inflows?|outflows?|regulatory\s+status)\b[^?]{0,30}\b(?:current|right\s+now|live|latest|today(?:'s)?|now)\b|поточн\p{L}*\s+(?:цін|курс|новин|поток|регуляторн\p{L}*\s+статус)|останн\p{L}*\s+новин|сьогоднішн\p{L}*\s+(?:цін|курс|приплив|відток)|прямо\s+зараз/iu;
...
function decision(
  ...
  return {
    ...
    needsCurrentInformation: route === "live_unsupported",
  ...
  async resolve(question: string): Promise<AiRouterDecision> {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 408 || response.status === 429 || response.status >= 500) throw new TransientRouterError("Temporary router failure.");
        ...
        if (usage.estimatedCostUsd > maxCostUsd) throw new Error("Router cost limit exceeded.");
        ...
        return validateRouterDecision({ ...routed, language: languageOf(question), databaseIntent: mergedDatabaseIntent });
      } catch (error) {
        lastError = error;
        const transient = error instanceof TransientRouterError
