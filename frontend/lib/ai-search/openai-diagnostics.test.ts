...
import { buildPlainResponseRequest, buildStructuredResponseRequest, OpenAiRequestValidationError } from "@/lib/ai-search/openai-request-factory";
...
async function rejectedDiagnostic(response: Response) {
  ...
  await expect(requestOpenAiResponse({
    apiKey: "secret-key",
    model: "gpt-5-mini",
    body: buildPlainResponseRequest({ model: "gpt-5-mini", input: "private question", instructions: "private prompt", maxOutputTokens: 50 }),
    timeoutMs: 50,
    fetchImpl,
    retryBackoffMs: 0,
...
describe("safe OpenAI diagnostics", () => {
  ...
  it.each([
    ...
    const body = buildPlainResponseRequest({ model: "gpt-5-mini", input: "private input", instructions: "private instructions", maxOutputTokens: 50 });
  ...
  it("returns a successful response without diagnostic logging", async () => {
    ...
    const response = await requestOpenAiResponse({
      ...
      body: buildPlainResponseRequest({ model: "gpt-5-mini", input: "input", instructions: "instructions", maxOutputTokens: 50 }),
      timeoutMs: 50,
  ...
  it("rejects a tampered schema before fetch", async () => {
    ...
    })).rejects.toBeInstanceOf(OpenAiRequestValidationError);
