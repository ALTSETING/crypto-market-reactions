...
export function runMultiHorizonAnalytics(events: readonly AnalyticsEvent[], intent: AiSearchIntent): MultiHorizonAnalyticsResult {
  const results = HORIZONS.map((horizon) => {
    ...
    return {
      ...
      negativePercent: shareResult.kind === "share" ? shareResult.negativePercent : null,
      neutralPercent: shareResult.kind === "share" ? shareResult.neutralPercent : null,
  ...
  return {
    ...
    rows: results.map((row) => ({
      ...
      neutralPercent: row.neutralPercent,
