...
import { deterministicRouterDecision, OpenAiResearchRouter } from "@/lib/ai-search/router";
import { AI_ROUTER_JSON_SCHEMA, validateRouterDecision } from "@/lib/ai-search/router-schema";
...
describe("AI Research router schema and provider", () => {
  it("accepts only exact, internally consistent allowlisted decisions", () => {
    expect(Object.keys(AI_ROUTER_JSON_SCHEMA.properties)).toEqual([
      "route", "language", "generalTopic", "databaseIntent", "needsHistoricalAnalysis",
      "needsCurrentInformation", "clarificationQuestion", "refusalReason",
    ]);
    ...
    expect(() => validateRouterDecision({ ...VALID_GENERAL, needsGeneralExplanation: true })).toThrow();
    expect(() => validateRouterDecision({ ...VALID_GENERAL, needsCurrentInformation: true })).toThrow();
  ...
  it("marks current regulatory requests as live-only with the exact current-information flag", () => {
    expect(deterministicRouterDecision("What is the current regulatory status of a Bitcoin ETF?")).toMatchObject({
      route: "live_unsupported",
      needsHistoricalAnalysis: false,
  ...
  it("never lets AI override explicit database constraints", async () => {
    const aiDecision = {
      ...
      needsHistoricalAnalysis: true, needsCurrentInformation: false, clarificationQuestion: null, refusalReason: null,
    };
